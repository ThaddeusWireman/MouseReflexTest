...
# that the user clicked and the average time per button click rounded to 3 decimal points are displayed. The 
# "Hard" mode is a significantly smaller button. 
...
def introduction():
    ...
    userLabel = tk.Label(master=userInfoFrame, text="\n\n\n\n\n\nPlease enter a username: ", font=("Calibra", 20), pady=17)
    userLabel.pack(anchor="center")
    ...
    errorMessageLabel.pack(anchor="center")
    ...
    gameInfoLabel.pack(anchor="center")
...
def check_entry():
    ...
    if username == "":
        ...
        errorMessageLabel.config(text="Username cannot be more than 12 characters long")
        return
    difficulty_choice()
...
def difficulty_choice():
    ...
    normalButton = tk.Button(master=levelFrame,text="Normal",height=6, width=15,font=("Calibra", 12), pady=20,
        command=lambda: game_layout(gameSettings['username'], gameSettings['normal']))
    
    hardButton = tk.Button(master=levelFrame,text="Hard",height=6, width=15,font=("Calibra", 12), pady=20,
...
def timer():
    ...
    if timeLabel['text'] == 0:
        ...
        if pointsLabel['text'] == 0:
            zeroScoreLabel = tk.Label(master=gameFrame, text=f"\n\n\nYOU DIDN'T CLICK ANY TARGETS! \nYour average isn't calculatable.",
        ...
        # calculates the average time per target label, score label, and reset button
            ...
            scoreLabel = tk.Label(master=gameFrame, text=f"\n\n\nYour score was: {pointsLabel['text']}", font=("Calibra", 20))
            resetButton = tk.Button(master=gameFrame, text="Reset?", command=reset, font=("Calibra", 12))
            quitButton = tk.Button(master=gameFrame, text="Quit", command=quit_program, font=("Calibra", 12))
            ...
            scoreLabel.grid(row=0, column=0, pady=10, sticky="nsew")
            aveTimeLabel.grid(row=1, column=0, pady=10, sticky="nsew")
            resetButton.grid(row=2, column=0, pady=10, sticky="nsew")
            quitButton.grid(row=3, column=0, pady=10, sticky="nsew")
            return 0
...
# sizes and centers the tkinter window horizontally and pins it to the top of the screen
windowWidth = 1100
windowHeight = 680
